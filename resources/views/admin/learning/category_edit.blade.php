          ...
          <div class="col-lg-12">
            <div class="card">
              <div class="card-header" style="background: #ddd;">  
                <div class="row">   
                  <div class="col-md-6"> 
                    <div class="m-0 h5"> แก้ไขข้อมูลประเภทแหล่งเรียนรู้ </div>
                  </div>
                  <div class="col-md-6 text-right"> 
                    <a href="{{ route('learningcategory.list') }}" class="m-0 h5"><i class="fe-chevron-left"></i> ย้อนกลับ </a> 
                  </div>   
                </div>
              </div>
              <div class="card-body">  
                ...
                @endif  
                <form method="POST" action="{{ route('save.learningcategory') }}" id="form" enctype="multipart/form-data">
                  @csrf  
                  <input type="hidden" id="statusData" name="statusData" value="U">
                  <input type="hidden" id="id" name="id" value="{{ $data['learningcategory_find']->id }}">
                  ...
                  <div class="row"> 
                    <div class="col-md-6 form-group"> 
                      ...
                      value="{{ $data['learningcategory_find']->name }}"
                    ...
                    <div class="col-md-6 form-group"> 
                      <label class="ml-1" for="detail"> รายละเอียดประเภทแหล่งเรียนรู้ <span class="text-danger">*</span></label>
                      <input id="detail" type="text" class="form-control form-control-lg @error('detail') invalid @enderror" name="detail"  
                      value="{{ $data['learningcategory_find']->detail }}"
                    ...
                    <div class="col-md-6 form-group"> 
                      ...
                      <div class="cc-selector"> 
                        <input id="status1" type="radio" name="status" value="0" {{ $data['learningcategory_find']->deleted_at == true ? $data['learningcategory_find']->deleted_at == 0 ? "checked" : ""  : "checked"  }}/>
                        <label class="drinkcard-cc bg-success" for="status1"> เปิดการแสดงผล </label>  

                        <input id="status2" type="radio" name="status" value="1" {{ $data['learningcategory_find']->deleted_at == 1 ? "checked" : "" }}/>
                  ...
                  <div class="row"> 
                    <div class="col-md-12 form-group text-right">  
                      <a href="{{ route('learningcategory.list') }}" class="btn btn-lg btn-dark waves-effect waves-light"><i class="fe-chevron-left"></i> ย้อนกลับ </a>  
                      <button type="button" class="btn btn-lg btn-danger waves-effect waves-light" id="close" data-id="{{ $data['learningcategory_find']->id }}"> <i class="mdi mdi-delete"></i> ยกเลิกข้อมูล </button>
...
<script> 
  ...
  $(document).on('click', '#close', function(event) { 
    ...
    Swal.fire({
        ...
        if (result.value) {  
          $.post("{{ route('close.learningcategory') }}", {
            _token: "{{ csrf_token() }}",  
            id: id, 
          })
          .done(function(data, status, error){   
            if(error.status==200){     
              if(data==true){
                location.href = "{{ route('learningcategory.list') }}";
              } 
            }
          })
          ...
          vthis[0].innerHTML='ยกเลิกข้อมูล'; 
        }
    }); 
  }); 
</script>
